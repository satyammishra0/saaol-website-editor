...
$id = $_GET['id'];
$query = "SELECT * FROM `state_seo_details` WHERE `state_seo_id`= :id";
            ...
            <form id="editMetaForm">
                ...
                <div class="mb-5  rounded-lg  flex justify-between items-center text-white" id="edit-city-parent">
                    <p class="text-lg" id="city-name-text">
                        <?php
                        $currentStatesQuery = "SELECT * FROM `state_seo_details` WHERE `state_seo_id`= :id";
                        $prepCurrentStateQuery = $conn->prepare($currentStatesQuery);
                        $prepCurrentStateQuery->bindParam(":id", $id);
                        try {
                            //code...
                            $prepCurrentStateQuery->execute();
                        } catch (PDOException $th) {
                            throw $th;
                        }
                        $currentStateName = $prepCurrentStateQuery->fetch(PDO::FETCH_ASSOC);
                        echo $currentStateName['state_name'];
                        ?>
                    </p>
                ...
                <div id="select-city-parent">
                    <label for="select" class="block mb-2">Edit State Details:</label>
                ...
                <div class="input-container">
                    ...
                    <input type="text" id="banner-img-alt" name="banner-img-alt" class="form-input" value="<?= $result['banner_alt_tag']; ?>">
                </div>
                ...
                <div class="input-container">
                    ...
                    <input type="text" id="meta-desc" name="meta-desc" class="form-input" value="<?= $result['meta_desc']; ?>">
                ...
                <!-- Local Scheme field -->
                <div class="input-container">
                    <label for="local-schema" class="form-label">State Content</label>
                    <textarea cols="30" rows="10" type="text" id="state-content" name="state-content" class="form-input"><?= $result['state_content']; ?></textarea>
                </div>
